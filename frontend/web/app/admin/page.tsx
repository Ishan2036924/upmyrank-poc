...
import {
  ...
  TrendingDown, Minus, Globe, Clock, Database, Star, Download,
} from 'lucide-react'
import {
...
// ── Diagnostic check explanations ──────────────────────────────────────────
const CHECK_EXPLANATIONS: Record<string, { what: string; why: string }> = {
  table_accessibility: {
    what: 'Verifies that all 13 database tables can be queried.',
    why: 'If any table is inaccessible, the affected feature silently breaks — no 500 error, just missing data.',
  },
  judge_evaluations_recent: {
    what: 'Checks whether the LLM quality-judge wrote evaluation rows in the last 24h.',
    why: '0 rows is normal if no study sessions ended recently. If students were active and you still see 0, the judge pipeline may be broken (check Render logs for errors in _run_judge_for_session).',
  },
  response_feedback_recent: {
    what: 'Checks whether students clicked thumbs up/down on AI responses in the last 24h.',
    why: '0 is expected with no active real users. If real students are chatting and never giving feedback, the thumbs buttons may not be rendering correctly.',
  },
  conversation_turn_quality_active: {
    what: 'Checks whether the per-turn Socratic quality scorer wrote rows in the last 24h.',
    why: '2 rows means 2 conversation turns were scored — healthy if there were sessions. 0 means no tutoring activity (or the scorer is not firing).',
  },
  null_embeddings: {
    what: 'Counts knowledge chunks that are missing their vector embedding.',
    why: 'A chunk with no embedding cannot be retrieved by RAG — it is effectively invisible to the AI tutor. Any missing chunk = a gap in the knowledge base.',
  },
  orphaned_doubt_sessions: {
    what: 'Counts doubt_sessions that have no linked doubt_block.',
    why: 'These are usually leftover rows from diagnostic harness runs or aborted sessions. They have no impact on real users but inflate DB size. Safe to clean up with diag_cleanup_test_accounts.py.',
  },
  slow_sessions: {
    what: 'Counts RAG retrieval calls that took more than 10 seconds in the last 7 days.',
    why: 'Mostly caused by Render cold-starts (first request after spin-down takes 22–116s). UptimeRobot keep-alive should reduce this. Persistent high counts → investigate Supabase pgvector query plans.',
  },
  redis_connectivity: {
    what: 'Sends a PING to the Upstash Redis instance and checks for a response.',
    why: 'Redis powers hot-context cache, semantic cache, and rate-limiter state. If Redis is down, all three degrade silently (per Rule #3) — no crash, but responses may be slower and rate-limiting inactive.',
  },
}

// ── Diagnostics Markdown export ─────────────────────────────────────────────
function exportDiagnosticsMarkdown(data: any) {
  if (!data) return
  const statusIcon = data.status === 'ok' ? '✅' : data.status === 'warning' ? '⚠️' : '❌'
  const ts = new Date(data.ran_at).toLocaleString()
  const lines: string[] = [
    '# UpMyRank — System Diagnostics Report',
    '',
    `**Generated:** ${ts}`,
    `**Overall Status:** ${statusIcon} ${data.status.toUpperCase()}`,
    '',
    '---',
    '',
    '## Checks',
  ...
  for (const c of data.checks) {
    ...
    const exp = CHECK_EXPLANATIONS[c.name]
    lines.push(`### ${icon} ${c.name.replace(/_/g, ' ')}`)
    lines.push('')
    lines.push(`**Result:** ${c.detail}`)
    if (exp) {
      lines.push('')
      lines.push(`**What it checks:** ${exp.what}`)
      lines.push('')
      lines.push(`**Why it matters:** ${exp.why}`)
    }
    lines.push('')
    lines.push('---')
    lines.push('')
  }
  lines.push('*Report auto-generated by UpMyRank Admin Diagnostics.*')
  const blob = new Blob([lines.join('\n')], { type: 'text/markdown;charset=utf-8' })
  const url = URL.createObjectURL(blob)
  const a = document.createElement('a')
  const dateStr = new Date().toISOString().slice(0, 10)
  a.href = url; a.download = `upmyrank-diagnostics-${dateStr}.md`
            ...
            {activeSection === 'diagnostics' && (
              <Section title="System Diagnostics" icon={Stethoscope} onRefresh={() => {}} loading={false}>
                <motion.div variants={fadeUp} className="flex flex-wrap items-center gap-3 mb-6">
                  ...
                  {diagnosticsData && (
                    <button onClick={() => exportDiagnosticsMarkdown(diagnosticsData)}
                      className="flex items-center gap-2 px-4 py-3 bg-white border border-slate-200 rounded-2xl text-sm font-semibold text-slate-600 hover:border-purple-300 hover:text-purple-600 transition-all active:scale-95 shadow-[0_2px_8px_rgb(0,0,0,0.04)]">
                      <Download className="w-4 h-4" />
                      Download Report
                    </button>
                  )}
                  <p className="text-xs text-slate-400 w-full">Checks all tables, Redis, latency, and data pipeline health.</p>
                </motion.div>
                      ...
                      {diagnosticsData.checks.map((c: any) => {
                        ...
                        return (
                          <motion.div key={c.name} variants={fadeUp}
                            className={`bg-white/80 border ${borderColor} rounded-xl px-4 py-3.5 shadow-[0_2px_8px_rgb(0,0,0,0.03)]`}>
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                {exp && (
                                  <div className="mt-2 space-y-1 border-t border-slate-100 pt-2">
                                    <p className="text-xs text-slate-500"><span className="font-semibold text-slate-600">What: </span>{exp.what}</p>
                                    <p className="text-xs text-slate-500"><span className="font-semibold text-slate-600">Why: </span>{exp.why}</p>
                                  </div>
                                )}
                              </div>
                            </div>
