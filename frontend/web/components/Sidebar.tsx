...
export default function Sidebar() {
  ...
  const slimNav = (
    <div className="flex flex-col h-full items-center py-5 gap-2">
      {/* Logo / Avatar */}
      <div className="w-9 h-9 rounded-full bg-gradient-to-br from-violet-500 to-indigo-600 flex items-center justify-center text-xs font-bold text-white select-none shadow-md mb-4 flex-shrink-0">
        {initials}
      </div>
      ...
      <button
        ...
        disabled={loading}
        title="Refresh data"
        className="w-10 h-10 rounded-2xl flex items-center justify-center text-slate-400 hover:text-slate-700 hover:bg-white/70 transition-all disabled:opacity-40"
      >
        <RefreshCw className={`h-4 w-4 ${loading ? 'animate-spin' : ''}`} style={{ width: 16, height: 16 }} />
      </button>
      <Link
        href="/settings"
        title="Settings"
        className="w-10 h-10 rounded-2xl flex items-center justify-center text-slate-400 hover:text-slate-700 hover:bg-white/70 transition-all"
      >
  ...
  return (
    <>
      ...
      <aside className="hidden md:flex fixed left-0 top-0 h-full w-[68px] z-40 flex-col items-center">
        {/* Glassmorphic pill */}
        <div className="m-3 flex-1 w-full rounded-3xl bg-white/70 backdrop-blur-xl border border-white/60 shadow-xl shadow-slate-200/50 overflow-hidden">
          {slimNav}
        </div>
      </aside>
